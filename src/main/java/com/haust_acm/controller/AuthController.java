...
public class AuthController extends ABaseController{
    ...
    public ResponseVO login(@RequestBody LoginRequest request,  HttpServletResponse response,HttpSession  session) throws BusinessException {
        ...
        if(user == null ){
            ...
            throw new BusinessException("账号处于封禁状态", ResponseCodeEnum.CODE_403.getCode());
        }else if(!Objects.equals(user.getPassword(), query.getPassword())){
            throw new BusinessException("密码错误", ResponseCodeEnum.CODE_600.getCode());
        }
    ...
    public ResponseVO register(@RequestBody User user,  HttpServletResponse response,HttpSession  session) throws BusinessException {
        user.getUserName();
        user.getEmail();
        user.getPassword();
        UserQuery userQueryEmail = new UserQuery();
        userQueryEmail.setEmail(user.getEmail());
        if(userService.findCountByParam(userQueryEmail) > 0){
            throw new BusinessException("邮箱已存在", ResponseCodeEnum.CODE_601.getCode());
        }else{
            return getSuccessResponseVO(userService.add(user),"注册成功，请返回登录界面进行登录");
        }
    }
}
